#!/usr/bin/env python3
"""
Sn₇Pt₄O₃/Al₂O₃ 双模式轨迹处理脚本
功能：
1. 基于 z 坐标自动识别 Support (240 原子) 和 Cluster (14 原子)
2. 验证 unwrap 正确性（检测跨盒跳跃）
3. 生成两套居中轨迹：
   - surface_centered.xyz (基底居中，用于表面迁移分析)
   - cluster_centered.xyz (团簇居中，用于内部重排分析)
"""
...
class TrajectoryAnalyzer:
    def __init__(self, xyz_file, support_n=240, cluster_n=14):
        """
        参数:
            xyz_file: 输入的 unwrap XYZ 轨迹文件
            support_n: Support 原子数 (默认 240)
            cluster_n: Cluster 原子数 (默认 14)
        """
